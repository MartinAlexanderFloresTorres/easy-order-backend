...
import TableOrder, { TABLE_ORDER_STATUS, TABLE_PAYMENT_STATUS } from '../../models/v1/TableOrder.js';
...
export const create = async (req, res) => {
  try {
    const { user } = req;
    const { restaurantId, coupon, discount, total, subTotal, items } = req.body;
...
export const getOrdersByUser = async (req, res) => {
  try {
    ...
    const orders = await TableOrder.find({ user: user._id, isActive: true })
      .select('-__v -isActive')
      .populate([
        {
          path: 'restaurant',
...
export const cancelOrder = async (req, res) => {
  try {
    ...
    const order = await TableOrder.findOne({ _id: orderId, user: user._id, isActive: true });

    if (!order) {
      return res.status(404).json({ message: 'La orden no existe' });
    }

    // Si la orden ya a sido rechazada no se puede cancelar
    if (order.status === TABLE_ORDER_STATUS.REJECTED) {
    ...
    // Si la orden no está en estado EMITTED no se puede cancelar
    if (order.status !== TABLE_ORDER_STATUS.EMITTED) {
      return res.status(400).json({
    ...
    if (order.status === TABLE_ORDER_STATUS.CANCELLED) {
      return res.status(400).json({ message: 'La orden ya ha sido cancelada', status: order.status });
    }

    order.status = TABLE_ORDER_STATUS.CANCELLED;
...
export const changeOrderStatus = async (req, res) => {
  try {
    ...
    if (!Object.values(TABLE_ORDER_STATUS).includes(status)) {
      return res.status(400).json({ message: 'El estado no es válido' });
    }

    const order = await TableOrder.findOne({ _id: orderId, isActive: true });
    if (!order) {
      return res.status(404).json({ message: 'La orden no existe' });
    }
    ...
    if (order.paymentStatus === TABLE_PAYMENT_STATUS.PAID) {
...
export const getOrdersByRestaurant = async (req, res) => {
  try {
    ...
    const orders = await TableOrder.find({ restaurant: restaurantId, isActive: true })
...
export const getOrderById = async (req, res) => {
  try {
    ...
    const order = await TableOrder.findOne({ _id: orderId, restaurant: restaurantId, isActive: true })
      .select('-restaurant -__v -isActive')
      .populate([
        {
          path: 'user',
...
export const payOrder = async (req, res) => {
  try {
    ...
    if (
      order.status === TABLE_ORDER_STATUS.EMITTED ||
      order.status === TABLE_ORDER_STATUS.REJECTED ||
      order.status === TABLE_ORDER_STATUS.CANCELLED
    ) {
    ...
    if (order.paymentStatus === TABLE_PAYMENT_STATUS.CANCELLED) {
    ...
    order.paymentStatus = TABLE_PAYMENT_STATUS.PAID;
    order.status = TABLE_ORDER_STATUS.COMPLETED;
