...
/**
 *
 * @param {*} { email, name, token }
 * @description Genera un email de confirmacion de cuenta
 * @example emailRegisterRestaurant({
...
export const emailRegisterRestaurant = async ({ email, name, token }) => {
  const HTML = `
  <!DOCTYPE html>
  <html lang="es">
    <head>
      ...
      <title>Cree su nuevo resturante</title>
      <style>
        body {
    ...
    <body>
      <div class="card">
        <div class="container" style="margin-bottom: 40px">
          <h2 style="text-transform: uppercase">Cree su nuevo resturante</h2>
          <p>Hola ${name}, Gracias por subscribirte a Orden Facil. Para crear tu nuevo restaurante, por favor ingresa al siguiente enlace</p>
  
         
  
          <div style="margin: 20px 0">
            <a class="boton" style="text-align: center; min-width: fit-content" href="${process.env.FRONTEND_URL}/restaurant/new/${token}"target="_blank">
              Crear Restaurante
            </a>
          </div>
  
          <p style="color: #ccc !important; font-size: 14px; text-align: center; line-height: 1.2">
            Si no has solicitado crear un restaurante, por favor ignora este correo electrónico o envíe un correo electrónico a nuestro equipo de soporte para obtener ayuda.
          </p>
  ...
  const msg = {
    ...
    subject: 'Crear restaurante - Orden Facil',
    text: 'Orden Facil',
    html: HTML,
  };
  sgMail
